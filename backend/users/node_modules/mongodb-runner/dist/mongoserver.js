...
class MongoServer {
    ...
    static async _start({ ...options }) {
        const srv = new MongoServer();
        if (!options.docker) {
            const dbPath = path_1.default.join(options.tmpDir, `db-${srv.uuid}`);
        ...
        try {
            ...
            if (buildInfoError) {
                (0, util_1.debug)('failed to get buildInfo', buildInfoError);
            }
        }
        catch (err) {
            await srv.close();
    ...
    async _ensureMatchingMetadataColl(client, mode) {
        ...
        const insertedInfo = (0, util_1.pick)(this.serialize(), [
            '_id',
            'pid',
            'port',
            'dbPath',
        ...
        (0, util_1.debug)('ensuring metadata collection entry', insertedInfo, { isMongoS });
        if (mode === 'insert-new') {
            await runnerColl.insertOne(insertedInfo);
            (0, util_1.debug)('inserted metadata collection entry', insertedInfo);
            this.hasInsertedMetadataCollEntry = true;
        }
        else {
            if (!this.hasInsertedMetadataCollEntry) {
                (0, util_1.debug)('skipping metadata collection match check as we never inserted metadata');
                return;
            }
            const match = await runnerColl.findOne();
            (0, util_1.debug)('read metadata collection entry', insertedInfo, match);
            if (!match) {
                throw new Error('Cannot find mongodbrunner entry, assuming that this instance was not started by mongodb-runner');
            }
            if (match._id !== insertedInfo._id) {
                throw new Error(`Mismatched mongodbrunner entry: ${JSON.stringify(match)} !== ${JSON.stringify(insertedInfo)}`);
            }
        }
    }
    async _populateBuildInfo(mode) {
        try {
            ...
            (0, util_1.debug)('failed to get buildInfo, treating as closed server', err);
            return err;
        }
    ...
    async withClient(fn, clientOptions = {}) {
        const client = await mongodb_1.MongoClient.connect(`mongodb://${this.hostport}/`, {
